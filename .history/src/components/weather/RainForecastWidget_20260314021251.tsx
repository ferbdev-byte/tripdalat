  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2 text-pine-green">
        <CloudRain size={18} />
        <h3 className="text-base">Dự báo mưa theo POI</h3>
      </div>
      ...
      {chartData.length > 0 && (
        <div className="rounded-2xl border border-hydrangea-blue/30 bg-white/65 p-4 backdrop-blur-sm">
          <div className="mb-2 flex items-center justify-between gap-2">
            <p className="text-xs font-medium text-text/70">Humidity & Cloud cover (4h - 5h - 6h)</p>
            {hasHighCloudHuntChance && (
              <span className="rounded-full bg-hydrangea-blue/25 px-2.5 py-1 text-xs font-medium text-pine-green">
      ...
      {forecasts.length === 0 ? (
        <p className="text-sm text-text/70">Chưa có POI trong itinerary để phân tích dự báo mưa.</p>
      ) : (
        <ul className="space-y-2 rounded-2xl border border-white/25 bg-white/55 p-4 text-sm text-text/85">
          {forecasts
            .sort((a, b) => b.rainProbability - a.rainProbability)
            .map((item) => (
              <li key={item.placeId} className="flex items-center justify-between gap-3">
                <span>{item.placeName}</span>
                <span className="rounded-full bg-mist-gray px-2 py-0.5 text-xs text-pine-green">{item.rainProbability}%</span>
              </li>
            ))}
        </ul>
      )}
      ...
      {hasHighRain && (
        <div className="rounded-2xl border border-pine-green/20 bg-white/60 p-4 text-sm text-text/80">
