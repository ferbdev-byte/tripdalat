'use client';

import { ChevronDown, CloudRain, Droplets, ThermometerSun } from 'lucide-react';
import { useEffect, useMemo, useRef, useState } from 'react';
import { Line, LineChart, ResponsiveContainer, Tooltip, XAxis, YAxis } from 'recharts';
import type { Place, WeatherPoint } from '../../data/mockData';

type IndoorCafe = {
  id: string;
  name: string;
};

type RainForecastWidgetProps = {
  places: Place[];
  hourlyWeather: WeatherPoint[];
  highRainThreshold?: number;
  onRainStatusChange?: (isRaining: boolean) => void;
  onHourlyRainChange?: (hourlyRain: Record<string, number>) => void;
};

const randomOffset = (seed: number) => {
  const value = (seed * 17) % 9;
  return value - 4;
};

const clamp = (value: number, min: number, max: number) => {
  return Math.min(max, Math.max(min, value));
};

export function RainForecastWidget({
  places,
  hourlyWeather,
  highRainThreshold = 60,
  onRainStatusChange,
  onHourlyRainChange,
}: RainForecastWidgetProps) {
  const [refreshCount, setRefreshCount] = useState(0);
  const [pullDistance, setPullDistance] = useState(0);
  const [now, setNow] = useState(() => new Date());
  const [isDetailsOpen, setIsDetailsOpen] = useState(false);
  const touchStartY = useRef<number | null>(null);

  useEffect(() => {
    const timer = window.setInterval(() => {
      setNow(new Date());
    }, 1000);

    return () => {
      window.clearInterval(timer);
    };
  }, []);

  const forecasts = useMemo(() => {
    const averageRain =
      hourlyWeather.length > 0
        ? Math.round(hourlyWeather.reduce((sum, point) => sum + point.rainProbability, 0) / hourlyWeather.length)
        : 0;

    return places.map((place, index) => ({
      placeId: place.id,
      placeName: place.name,
      rainProbability: Math.max(0, Math.min(100, averageRain + randomOffset(index + refreshCount + 1))),
    }));
  }, [hourlyWeather, places, refreshCount]);

  const indoorCafes = useMemo<IndoorCafe[]>(() => {
    return places
      .filter((place) => place.category === 'cafe' && place.is_indoor)
      .map((place) => ({ id: place.id, name: place.name }));
  }, [places]);

  const handleTouchStart = (event: React.TouchEvent<HTMLDivElement>) => {
    if (window.scrollY > 0) return;
    touchStartY.current = event.touches[0]?.clientY ?? null;
  };

  const handleTouchMove = (event: React.TouchEvent<HTMLDivElement>) => {
    if (touchStartY.current === null) return;
    const currentY = event.touches[0]?.clientY ?? 0;
    const delta = Math.max(0, currentY - touchStartY.current);
    setPullDistance(Math.min(delta, 120));
  };

  const handleTouchEnd = () => {
    if (pullDistance > 80) {
      setRefreshCount((value) => value + 1);
    }
    setPullDistance(0);
    touchStartY.current = null;
  };

  const maxRain = useMemo(() => {
    if (forecasts.length === 0) return 0;
    return Math.max(...forecasts.map((item) => item.rainProbability));
  }, [forecasts]);

  const chartData = useMemo(() => {
    if (hourlyWeather.length === 0) {
      return [] as Array<{ hour: string; humidity: number; cloudCover: number; windSpeed: number }>;
    }

    return hourlyWeather
      .map((point) => ({
        hour: point.hour,
        humidity: point.humidity,
        cloudCover: point.cloudCover,
        windSpeed: point.windSpeed,
      }))
      .filter((point) => ['04', '05', '06'].includes(point.hour))
      .sort((a, b) => Number(a.hour) - Number(b.hour));
  }, [hourlyWeather]);

  const hourlyAverage = useMemo(() => {
    const nextValue: Record<string, number> = {};
    for (const point of hourlyWeather) {
      nextValue[point.hour] = point.rainProbability;
    }
    return nextValue;
  }, [hourlyWeather]);

  const hasHighRain = maxRain >= highRainThreshold;

  const liveWeather = useMemo(() => {
    if (hourlyWeather.length === 0) {
      return {
        dateLabel: now.toLocaleDateString('vi-VN'),
        timeLabel: now.toLocaleTimeString('vi-VN'),
        temperature: 0,
        humidity: 0,
        rainProbability: 0,
        conditionLabel: 'Không có dữ liệu',
      };
    }

    const sorted = [...hourlyWeather].sort((a, b) => Number(a.hour) - Number(b.hour));
    const currentHourDecimal = now.getHours() + now.getMinutes() / 60;

    let previousPoint = sorted[0];
    let nextPoint = sorted[sorted.length - 1];

    for (let index = 0; index < sorted.length; index += 1) {
      const point = sorted[index];
      const pointHour = Number(point.hour);

      if (pointHour <= currentHourDecimal) {
        previousPoint = point;
      }

      if (pointHour >= currentHourDecimal) {
        nextPoint = point;
        break;
      }
    }

    const startHour = Number(previousPoint.hour);
    const endHour = Number(nextPoint.hour);
    const denominator = endHour - startHour === 0 ? 1 : endHour - startHour;
    const progress = clamp((currentHourDecimal - startHour) / denominator, 0, 1);
    const secondPulse = Math.sin((now.getSeconds() / 60) * Math.PI * 2);

    const interpolate = (start: number, end: number) => start + (end - start) * progress;

    const temperature = clamp(Math.round((interpolate(previousPoint.temperature, nextPoint.temperature) + secondPulse * 0.6) * 10) / 10, 10, 30);
    const humidity = clamp(Math.round(interpolate(previousPoint.humidity, nextPoint.humidity) + secondPulse * 1.2), 40, 100);
    const rainProbability = clamp(Math.round(interpolate(previousPoint.rainProbability, nextPoint.rainProbability)), 0, 100);
    const cloudCover = clamp(Math.round(interpolate(previousPoint.cloudCover, nextPoint.cloudCover)), 0, 100);

    let conditionLabel = 'Trời dịu và khô';
    if (rainProbability >= 70) {
      conditionLabel = 'Mưa cao, nên ưu tiên indoor';
    } else if (cloudCover >= 85) {
      conditionLabel = 'Nhiều mây, không khí mờ sương';
    } else if (humidity >= 88) {
      conditionLabel = 'Độ ẩm cao, cảm giác lạnh nhẹ';
    }

    return {
      dateLabel: now.toLocaleDateString('vi-VN', {
        weekday: 'long',
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
      }),
      timeLabel: now.toLocaleTimeString('vi-VN', {
        hour: '2-digit',
        minute: '2-digit',
        second: '2-digit',
      }),
      temperature,
      humidity,
      rainProbability,
      conditionLabel,
    };
  }, [hourlyWeather, now]);

  useEffect(() => {
    onRainStatusChange?.(hasHighRain);
    onHourlyRainChange?.(hourlyAverage);
  }, [hasHighRain, hourlyAverage, onHourlyRainChange, onRainStatusChange]);

  const hasHighCloudHuntChance = useMemo(() => {
    return chartData.some((point) => point.humidity > 90 && point.windSpeed <= 10);
  }, [chartData]);

  return (
    <section
      onTouchStart={handleTouchStart}
      onTouchMove={handleTouchMove}
      onTouchEnd={handleTouchEnd}
      className="space-y-3"
    >
      <div className="rounded-dalat border border-white/30 bg-white/68 p-4 shadow-[0_10px_24px_rgba(74,74,74,0.06)]">
        <div className="flex items-start justify-between gap-3">
          <div>
            <p className="text-[11px] text-[#4A4A4A]/60">Thời tiết hiện tại</p>
            <p className="mt-0.5 text-xs text-[#4A4A4A]/70">{liveWeather.dateLabel}</p>
          </div>

          <button
            type="button"
            onClick={() => setIsDetailsOpen((previous) => !previous)}
            className="inline-flex items-center gap-1 rounded-full border border-white/40 bg-white/75 px-2.5 py-1 text-[11px] text-[#4A4A4A]/80"
          >
            Chi tiết
            <ChevronDown className={`h-3.5 w-3.5 transition-transform ${isDetailsOpen ? 'rotate-180' : 'rotate-0'}`} />
          </button>
        </div>

        <div className="mt-3 flex items-end justify-between gap-4">
          <div>
            <p className="inline-flex items-center gap-1 text-[11px] text-[#4A4A4A]/60">
              <ThermometerSun className="h-3.5 w-3.5" /> Nhiệt độ
            </p>
            <p className="mt-1 text-3xl text-[#4A4A4A]" style={{ fontFamily: 'var(--font-heading), serif' }}>
              {liveWeather.temperature}°C
            </p>
          </div>

          <div className="space-y-1.5 text-right">
            <p className="inline-flex items-center gap-1 rounded-full border border-white/35 bg-white/70 px-2.5 py-1 text-xs text-[#4A4A4A]/80">
              <Droplets className="h-3.5 w-3.5 text-pine" /> {liveWeather.humidity}% ẩm
            </p>
            <p className="inline-flex items-center gap-1 rounded-full border border-white/35 bg-white/70 px-2.5 py-1 text-xs text-[#4A4A4A]/80">
              <CloudRain className="h-3.5 w-3.5 text-rose" /> {liveWeather.rainProbability}% mưa
            </p>
          </div>
        </div>

        <p className="mt-3 text-xs text-[#4A4A4A]/75">{liveWeather.conditionLabel}</p>
        {pullDistance > 0 && (
          <p className="mt-2 text-[11px] text-[#4A4A4A]/55">
            Đang kéo để cập nhật: {Math.round((pullDistance / 80) * 100)}%
          </p>
        )}
      </div>

      <div className="rounded-dalat border border-white/25 bg-white/55 px-3 py-2.5 text-xs text-[#4A4A4A]/75 shadow-[0_8px_20px_rgba(74,74,74,0.05)]">
        Rủi ro mưa cao nhất theo POI: <strong className="text-pine">{maxRain}%</strong>
      </div>

      {isDetailsOpen && (
        <>
          {chartData.length > 0 && (
            <div className="rounded-dalat border border-white/30 bg-white/65 p-3 sm:p-4 shadow-[0_10px_26px_rgba(74,74,74,0.07)] backdrop-blur-sm">
              <div className="mb-2 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs font-medium text-[#4A4A4A]/70">Humidity & Cloud cover (4h - 5h - 6h)</p>
                {hasHighCloudHuntChance && (
                  <span className="rounded-full bg-mist px-2.5 py-1 text-xs font-medium text-pine">
                    Cơ hội săn mây cao! ☁️
                  </span>
                )}
              </div>

              <div className="h-44 w-full sm:h-52">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={chartData}>
                    <XAxis dataKey="hour" tickFormatter={(value) => `${value}h`} tick={{ fontSize: 12 }} />
                    <YAxis domain={[0, 100]} tick={{ fontSize: 12 }} />
                    <Tooltip formatter={(value: number) => `${value}%`} labelFormatter={(label) => `${label}h`} />
                    <Line type="monotone" dataKey="humidity" stroke="#869484" strokeWidth={2} dot />
                    <Line type="monotone" dataKey="cloudCover" stroke="#A8DADC" strokeWidth={2} dot />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
          )}

          {forecasts.length === 0 ? (
            <p className="text-sm text-[#4A4A4A]/70">Chưa có POI trong itinerary để phân tích dự báo mưa.</p>
          ) : (
            <ul className="space-y-2 rounded-dalat border border-white/25 bg-white/60 p-3 sm:p-4 text-sm text-[#4A4A4A]/85 shadow-[0_8px_20px_rgba(74,74,74,0.06)]">
              {forecasts
                .sort((a, b) => b.rainProbability - a.rainProbability)
                .map((item) => (
                  <li key={item.placeId} className="flex items-center justify-between gap-3">
                    <span className="min-w-0 flex-1 truncate">{item.placeName}</span>
                    <span className="rounded-full bg-mist px-2 py-0.5 text-xs text-pine">{item.rainProbability}%</span>
                  </li>
                ))}
            </ul>
          )}

          {hasHighRain && (
            <div className="rounded-dalat border border-pine/30 bg-white/60 p-4 text-sm text-[#4A4A4A]/80 shadow-[0_8px_20px_rgba(74,74,74,0.06)]">
              <p>Tỉ lệ mưa đang cao, nên ưu tiên địa điểm indoor.</p>
              {indoorCafes.length > 0 ? (
                <ul className="mt-2 space-y-1 text-sm">
                  {indoorCafes.map((cafe) => (
                    <li key={cafe.id}>• {cafe.name}</li>
                  ))}
                </ul>
              ) : (
                <p className="mt-2">Chưa tìm thấy quán cafe indoor trong danh sách places.</p>
              )}
            </div>
          )}
        </>
      )}
    </section>
  );
}
