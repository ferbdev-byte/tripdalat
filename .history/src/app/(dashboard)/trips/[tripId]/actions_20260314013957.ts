...
export async function confirmRainSwapAction(
  ...
  const placeRows = (places ?? []) as PlaceNameRow[];

  const fromPlaceName = placeRows.find((place) => place.id === input.fromPlaceId)?.name ?? 'điểm A';
  const toPlaceName = placeRows.find((place) => place.id === input.toPlaceId)?.name ?? 'điểm B';
  ...
  const { error: logError } = await supabase
    .from('itinerary_swap_logs')
    .insert({
      itinerary_item_id: input.itineraryItemId,
      old_place_id: input.fromPlaceId,
      new_place_id: input.toPlaceId,
      reason: input.reason,
  ...
  if (logError) {
    return {
      ok: false,
      message: `Đã đổi điểm nhưng không ghi được log: ${logError.message}`,
    };
  }
