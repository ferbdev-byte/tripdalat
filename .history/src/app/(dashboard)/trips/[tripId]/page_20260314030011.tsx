...
import { BrainCircuit, CloudDrizzle, Compass, MapPin, Sparkles, Umbrella } from 'lucide-react';
...
const DalatMap = dynamic(
  () => import('../../../../components/map/DalatMap').then((module) => module.DalatMap),
  {
    ssr: false,
    loading: () => (
      <div className="flex h-[340px] items-center justify-center rounded-dalat border border-white/20 bg-white/35 text-sm text-[#4A4A4A]/70 backdrop-blur-md transition-all duration-700">
        Đang tải bản đồ...
      </div>
    ),
  },
...
export default function TripDashboardPage({ params }: PageProps) {
  ...
  return (
    <main className="relative min-h-screen space-y-10 overflow-hidden bg-[#FDFCFB] p-8 md:p-10">
      <div className="pointer-events-none absolute -left-20 top-10 h-72 w-72 animate-mist rounded-full bg-rose/20 blur-3xl" />
      <div className="pointer-events-none absolute right-0 top-28 h-80 w-80 animate-mist rounded-full bg-pine/15 blur-3xl [animation-delay:1s]" />
      ...
      <header className="relative space-y-3 transition-all duration-700">
        ...
        <h1 className="animate-fade-up text-4xl leading-tight text-[#4A4A4A] [animation-delay:120ms] md:text-6xl" style={{ fontFamily: 'var(--font-heading), serif' }}>
          Nhật ký chuyến đi mộng sương
        </h1>
        <p className="max-w-2xl animate-fade-up text-sm leading-7 text-[#4A4A4A]/75 [animation-delay:220ms]">
      ...
      <div className="relative grid grid-cols-1 gap-10 xl:grid-cols-3">
        ...
        <Card className="animate-fade-up bg-white/40 backdrop-blur-xl border border-white/20 rounded-dalat shadow-[0_16px_48px_rgba(74,74,74,0.08)] transition-all duration-700 [animation-delay:220ms] xl:col-span-1">
          <CardHeader className="p-8">
          ...
          <CardContent className="space-y-6 p-8 pt-0">
            <form onSubmit={handleAddCustomStop} className="animate-fade-up grid grid-cols-2 gap-2 rounded-dalat border border-white/25 bg-white/55 p-4 [animation-delay:140ms]">
              <input
                value={newStop.placeName}
                onChange={(event) => setNewStop((previous) => ({ ...previous, placeName: event.target.value }))}
                placeholder="Tên địa điểm"
                className="col-span-2 rounded-xl border border-white/40 bg-white/70 px-3 py-2 text-xs outline-none"
              />
              <input
                value={newStop.latitude}
                onChange={(event) => setNewStop((previous) => ({ ...previous, latitude: event.target.value }))}
                placeholder="Latitude"
                className="rounded-xl border border-white/40 bg-white/70 px-3 py-2 text-xs outline-none"
              />
              <input
              ...
              <input
                ...
                placeholder="Phút di chuyển"
                className="rounded-xl border border-white/40 bg-white/70 px-3 py-2 text-xs outline-none"
              />
              <label className="inline-flex items-center gap-2 rounded-xl border border-white/40 bg-white/70 px-3 py-2 text-xs text-[#4A4A4A]/80">
                <input
                  type="checkbox"
              ...
              <textarea
                ...
                className="col-span-2 h-16 rounded-xl border border-white/40 bg-white/70 px-3 py-2 text-xs outline-none"
              />
              <button type="submit" className="col-span-2 rounded-full bg-pine px-4 py-2 text-xs font-medium text-white transition hover:opacity-90">
            ...
            <div className="animate-fade-up flex flex-wrap gap-2 [animation-delay:160ms]">
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs transition ${focusMode === 'all' ? 'bg-pine text-white' : 'border border-white/30 bg-white/55 text-[#4A4A4A]/80'}`}
              >
                Tất cả
              </button>
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs transition ${focusMode === 'risky' ? 'bg-rose text-white' : 'border border-white/30 bg-white/55 text-[#4A4A4A]/80'}`}
              >
                Rủi ro mưa cao
              </button>
              <button
                ...
                className={`rounded-full px-3 py-1.5 text-xs transition ${focusMode === 'indoor' ? 'bg-pine text-white' : 'border border-white/30 bg-white/55 text-[#4A4A4A]/80'}`}
              >
                Chỉ indoor
              </button>
            </div>
              ...
              <div className="space-y-8">
                {filteredItinerary.map((item, index) => (
                  <div
                    ...
                    onClick={() => {
                      if (item.place?.id) setSelectedPoiId(item.place.id);
                    }}
                  >
                    ...
                    <div className={`rounded-dalat border bg-white/55 p-6 shadow-[0_10px_26px_rgba(74,74,74,0.09)] backdrop-blur-md ${selectedPoiId === item.place?.id ? 'border-pine/50 ring-1 ring-pine/40' : 'border-white/25'}`}>
                      ...
                      <div className="mt-3 flex items-center gap-2 text-xs text-[#4A4A4A]/75">
                        <span>Di chuyển:</span>
                        <input
                          type="number"
                          min={0}
                          ...
                          className="w-20 rounded-lg border border-white/40 bg-white/70 px-2 py-1 outline-none"
                        />
                        <span>phút</span>
                        <button
                          type="button"
                          ...
                          className="ml-auto rounded-full border border-pine/30 bg-white/70 px-3 py-1 text-[11px] text-pine transition hover:bg-white"
                        >
                          Trỏ trên map
                        </button>
                      </div>
            ...
            <div className="animate-fade-up rounded-dalat border border-pine/25 bg-white/60 p-5 text-sm text-[#4A4A4A] shadow-[0_10px_26px_rgba(74,74,74,0.07)] [animation-delay:280ms]">
              <div className="flex items-start justify-between gap-4">
                ...
                <button
                  ...
                  className="rounded-full bg-pine px-4 py-2 text-xs font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
                >
        ...
        <Card className="animate-fade-up bg-white/40 backdrop-blur-xl border border-white/20 rounded-dalat shadow-[0_16px_48px_rgba(74,74,74,0.08)] transition-all duration-700 [animation-delay:320ms] xl:col-span-1">
          <CardHeader className="p-8">
            <CardTitle className="inline-flex items-center gap-2" style={{ fontFamily: 'var(--font-heading), serif' }}>
