...
import { AlertTriangle, Backpack, BedDouble, BrainCircuit, Cat, CheckCircle2, ChevronDown, Circle, CloudDrizzle, Code2, Coffee, Compass, Dice6, ImageIcon, Loader2, MapPin, MapPinned, PlusCircle, Sparkles, Umbrella, UtensilsCrossed, Wallet, X } from 'lucide-react';
...
const loadDalatMapWithRecovery = async () => {
  try {
    return await import('../../../../components/map/DalatMap');
  } catch (error) {
    const message = error instanceof Error ? error.message : String(error);
    const isChunkLoadError = /ChunkLoadError|Loading chunk|dynamically imported module/i.test(message);

    if (typeof window !== 'undefined' && isChunkLoadError) {
      const retryKey = 'dalat-map-chunk-retry';
...
const DalatMap = dynamic(
  () => loadDalatMapWithRecovery().then((module) => module.DalatMap),
  {
    ssr: false,
    loading: () => (
...
export default function TripDashboardPage({ params }: PageProps) {
  ...
  const [roomMapsUrl, setRoomMapsUrl] = useState('');
  const [roomCheckInDate, setRoomCheckInDate] = useState(() => new Date().toISOString().split('T')[0]);
  const [roomManualPrice, setRoomManualPrice] = useState('');
  const [roomLookup, setRoomLookup] = useState<RoomLookupResult | null>(null);
  const [roomLookupError, setRoomLookupError] = useState<string | null>(null);
  const [isRoomLookupLoading, setIsRoomLookupLoading] = useState(false);
  ...
  const handleLookupRoomFromMaps = useCallback(async () => {
    const rawUrl = roomMapsUrl.trim();

    if (!rawUrl) {
      setRoomLookupError('Vui lòng dán link Google Maps hoặc Agoda của phòng.');
      return;
    }
    ...
    try {
      ...
      setRoomLookupError('Link không hợp lệ. Vui lòng kiểm tra lại URL.');
      return;
    }
    ...
    const isSupportedHost =
      ...
      parsedUrl.hostname.includes('agoda.com');

    if (!isSupportedHost) {
      setRoomLookupError('Hiện hỗ trợ link Google Maps hoặc Agoda.');
      return;
    }

    setIsRoomLookupLoading(true);
    setRoomLookupError(null);

    try {
      // Inject check-in date into Agoda URL if provided
      if (parsedUrl.hostname.includes('agoda.com') && roomCheckInDate.trim()) {
        parsedUrl.searchParams.set('checkIn', roomCheckInDate.trim());
      }
      const urlToFetch = parsedUrl.toString();

      const response = await fetch(`/api/google-maps-preview?url=${encodeURIComponent(urlToFetch)}`);
      ...
      if (!response.ok) {
        throw new Error(payload.error ?? 'Không thể lấy thông tin từ link này.');
      }

      const finalPrice = roomManualPrice.trim() || payload.priceText || null;
      const checkinLabel = roomCheckInDate.trim()
        ? new Date(roomCheckInDate + 'T00:00:00').toLocaleDateString('vi-VN', { day: '2-digit', month: '2-digit', year: 'numeric' })
        : null;
      setRoomLookup({
        title: payload.title || 'Phòng lưu trú',
        imageUrl: payload.imageUrl ?? null,
        priceText: finalPrice,
        minPriceText: roomManualPrice.trim() ? roomManualPrice.trim() : payload.minPriceText ?? finalPrice,
        maxPriceText: roomManualPrice.trim() ? roomManualPrice.trim() : payload.maxPriceText ?? finalPrice,
        roomOffers: (payload.roomOffers ?? [])
          .filter((item) => item && item.name && item.priceText)
          .map((item) => ({
            name: item.name as string,
            imageUrl: item.imageUrl ?? null,
            priceText: item.priceText as string,
          })),
        sourceUrl: payload.sourceUrl || rawUrl,
        fetchedAt: new Date().toISOString(),
        checkInDate: checkinLabel ?? undefined,
      });

      toast.success('Đã lấy thông tin phòng và sắp xếp giá tăng dần.');
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Không thể đọc dữ liệu từ link này.';
      setRoomLookupError(message);
      toast.error(message);
    } finally {
      setIsRoomLookupLoading(false);
    }
  }, [roomCheckInDate, roomManualPrice, roomMapsUrl]);
  ...
  return (
    <main className="relative min-h-screen space-y-5 overflow-hidden bg-[#FDFCFB] px-3 pb-[calc(7.25rem+env(safe-area-inset-bottom))] pt-4 sm:space-y-8 sm:p-6 md:space-y-10 md:p-10">
      ...
      <section className="space-y-3">
        <div className="inline-flex items-center gap-2 rounded-full border border-pine/30 bg-white/70 px-4 py-2 text-xs text-pine backdrop-blur-xl">
          <BedDouble className="h-3.5 w-3.5" />
          Check phòng lưu trú
        </div>

        <Card className="rounded-dalat border-2 border-pine/45 bg-white/65 shadow-[0_16px_40px_rgba(74,74,74,0.10)] ring-2 ring-pine/20 backdrop-blur-xl">
          <CardHeader className="p-5 sm:p-6">
            ...
            <CardDescription className="text-xs">Dán link Google Maps hoặc Agoda để lấy ảnh và thông tin giá phòng.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3 p-5 pt-0 sm:p-6 sm:pt-0">
            <label className="space-y-1 text-xs text-[#4A4A4A]/70">
              <span>Link Google Maps / Agoda</span>
              <input
                value={roomMapsUrl}
                onChange={(event) => setRoomMapsUrl(event.target.value)}
                placeholder="https://maps.app.goo.gl/... hoặc https://www.agoda.com/..."
                className="w-full rounded-2xl border border-white/40 bg-white/75 px-3 py-2.5 text-xs text-[#4A4A4A] outline-none"
              />
            </label>

            <label className="space-y-1 text-xs text-[#4A4A4A]/70">
              <span>Ngày check-in <span className="text-rose/70">*</span></span>
              <input
                type="date"
                value={roomCheckInDate}
                onChange={(event) => setRoomCheckInDate(event.target.value)}
                className="w-full rounded-2xl border border-white/40 bg-white/75 px-3 py-2.5 text-xs text-[#4A4A4A] outline-none"
              />
            </label>

            <label className="space-y-1 text-xs text-[#4A4A4A]/70">
              <span>Giá phòng (tuỳ chọn)</span>
              <input
                value={roomManualPrice}
                onChange={(event) => setRoomManualPrice(event.target.value)}
                placeholder="Ví dụ: 650.000đ/đêm"
                className="w-full rounded-2xl border border-white/40 bg-white/75 px-3 py-2.5 text-xs text-[#4A4A4A] outline-none"
              />
            </label>
            ...
            <button
              type="button"
              onClick={handleLookupRoomFromMaps}
              disabled={isRoomLookupLoading}
              className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-pine py-2.5 text-xs font-medium text-white transition hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
            >
              {isRoomLookupLoading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <ImageIcon className="h-3.5 w-3.5" />}
              {isRoomLookupLoading ? 'Đang lấy dữ liệu...' : 'Lấy ảnh & giá'}
            </button>

            {roomLookupError && (
              <p className="rounded-xl border border-rose/30 bg-rose/10 px-3 py-2 text-[11px] text-[#A36464]">{roomLookupError}</p>
            )}

            {roomLookup && (
              <div className="space-y-2 rounded-2xl border border-white/35 bg-white/70 p-3">
                <div className="h-32 overflow-hidden rounded-xl border border-white/35 bg-[#f3f3f3]">
                  {roomLookup.imageUrl ? (
                    <div
                      className="h-full w-full bg-cover bg-center"
                      style={{ backgroundImage: `url(${roomLookup.imageUrl})` }}
                      role="img"
                      aria-label={roomLookup.title}
                    />
                  ) : (
                    <div className="flex h-full items-center justify-center text-[11px] text-[#4A4A4A]/60">Không lấy được ảnh từ link này</div>
                  )}
                </div>
                <div className="flex flex-wrap items-center justify-between gap-1">
                  <p className="text-xs font-medium text-[#4A4A4A]">{roomLookup.title}</p>
                  {roomLookup.checkInDate && (
                    <span className="rounded-full bg-pine/10 px-2 py-0.5 text-[10px] text-pine">Check-in: {roomLookup.checkInDate}</span>
                  )}
                </div>
                <p className="inline-flex items-center gap-1 text-xs text-pine">
                  <Wallet className="h-3.5 w-3.5" />
                  {roomLookup.minPriceText && roomLookup.maxPriceText
                    ? `Khoảng giá: ${roomLookup.minPriceText} → ${roomLookup.maxPriceText}`
                    : roomLookup.priceText ?? 'Chưa đọc được giá, bạn có thể nhập tay ở ô trên'}
                </p>

                {roomLookup.roomOffers.length > 0 && (
                  <div className="space-y-1 rounded-xl border border-white/30 bg-white/75 p-2">
                    <p className="text-[11px] font-medium text-[#4A4A4A]/75">Phòng từ rẻ đến cao:</p>
                    <ul className="space-y-1">
                      {roomLookup.roomOffers.slice(0, 8).map((offer, index) => (
                        <li key={`${offer.name}-${index}`} className="flex items-center justify-between gap-2 text-[11px] text-[#4A4A4A]/85">
                          <span className="min-w-0 flex-1 truncate">{index + 1}. {offer.name}</span>
                          <span className="shrink-0 rounded-full bg-mist px-2 py-0.5 text-[10px] text-pine">{offer.priceText}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
                ...
                {roomLookup.roomOffers.length === 0 && roomLookup.sourceUrl.includes('agoda.com') && (
                  <p className="text-[11px] text-[#4A4A4A]/65">
                    Agoda chưa trả danh sách phòng/giá cho ngày đã chọn. Bạn thử đổi ngày check-in hoặc thêm giá tay để lưu nhanh.
                  </p>
                )}
                <a
                  href={roomLookup.sourceUrl}
                  target="_blank"
                  rel="noreferrer"
                  className="inline-flex text-[11px] text-pine underline"
                >
                  Mở link nguồn
                </a>
              </div>
