...
import { getMockTripById, type MockItineraryItem, type MockPlace } from '../../../../data/mock-trip';
...
export default function TripDashboardPage({ params }: PageProps) {
  ...
  const loadTripData = useCallback(async () => {
    ...
    const placesById = new Map<string, MockPlace>(tripData.places.map((place) => [place.id, place]));
    const mappedItems: ItineraryRow[] = tripData.itinerary_items.map((item: MockItineraryItem) => {
        ...
        <Card id="weather-hub" className="weather-hub rounded-[2rem] border-white/20 bg-white/40 backdrop-blur-md transition-all duration-700 xl:col-span-1">
          ...
          <CardContent className="p-8 pt-0">
            {tripId ? (
              <RainForecastWidget
                places={pois}
                hourlyWeather={getMockTripById(tripId).hourly_weather}
